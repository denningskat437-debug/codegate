...
set SMTP_PASSWORD=your_smtp_password_here
...
echo 'export SMTP_PASSWORD="xxx"' >> ~/.bashrc
```
...
```
1. config_manager.py          # 配置管理，基础模块
2. response_builder.py        # 响应构建，基础模块
3. hash_checker.py            # 哈希检测，基础模块
4. git_handler.py             # Git 操作，基础模块
5. code_analyzer.py           # 代码统计，基础模块
6. test_case_reader.py        # 测试用例读取，基础模块
7. user_mapping_reader.py     # 用户映射读取，基础模块
8. ai_risk_analyzer.py        # AI 风险评估，依赖 Claude SDK
9. test_executor.py           # 测试用例执行，依赖 Claude SDK
10. report_generator.py       # 报告生成，依赖 pandas
11. email_sender.py           # 邮件发送，依赖 SMTP
12. app.py                    # 整合所有模块
```
...
- 检查 Excel 格式是否正确（username, email 两列）
